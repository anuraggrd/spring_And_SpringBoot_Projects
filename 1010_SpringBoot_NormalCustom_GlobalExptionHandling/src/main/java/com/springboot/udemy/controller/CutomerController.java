package com.springboot.udemy.controller;
...
import com.springboot.udemy.service.CustomerService;
...
public class CutomerController {
	...
	public ResponseEntity<CustomerModel> getCustomerById(@PathVariable("id")int id) throws CustomerNotFound
	{
	...
	@RequestMapping(value = "/getCustById/{name}/{code}", method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<CustomerModel> getCustomerById(@PathVariable("name")String name,@PathVariable("code")String code) throws CustomerNotFound
	{
			CustomerModel customer = service.findByNameAndCustomerCode(name, code);
			return new ResponseEntity<CustomerModel>(customer,HttpStatus.OK);
	}
